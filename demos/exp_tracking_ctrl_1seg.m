% Tracking Control in one segment
function exp = exp_tracking_ctrl_1seg(varargin) 
    ...
    % define the segment
    segmentPnt = [1, 0.5; 7.5, 5.3];
    
     % The first path point is used as initial pose
    exp.robot.initialPose = [segmentPnt(1, :)+[-0.8, -0.5], 90 * pi / 180];
    ...
    exp.robot.controller = controller_ddrive_Lyapunov(segmentPnt);  
    ...
    exp.display.title = 'Kinematic Tracking Control';       
