% Experiment setup for Dynamic Tracking Control:
% Backstepping control of a Differential Drive Mobile Robot
function exp = exp_ktc_ddrive()
    exp = experiment_base('kinematic_tracking_control');
    
    % Some configuration options for the experiment
    poseProvider = 'slam'; %use 'exact','ekf' or 'slam'
    ...
    exp.robot.controller = controller_ddrive_Lyapunov(pathPoints);
    exp.robot.controller.K_x = 1;
    exp.robot.controller.K_phi = 1;
